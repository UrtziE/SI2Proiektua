...
public class BidaiakErreserbatuGUI extends JFrame {
	...
	JComboBox<Integer> seatsBox = new JComboBox<Integer>();
	DefaultComboBoxModel<Integer> seatslagun = new DefaultComboBoxModel<Integer>();
	...
	private JLabel seatKoplbl = new JLabel("");
	private JButton alertaBtn = new JButton(ResourceBundle.getBundle("Etiquetas").getString("AlertakIkusiGUI.AlertaSortu"));
	...
	private JButton btnErreserbatu=new JButton("Erreserbatu");
	...
	private String[] columnNamesRides = new String[] {
			...
			ResourceBundle.getBundle("Etiquetas").getString("FindRidesGUI.Price"),"",ResourceBundle.getBundle("Etiquetas").getString("TReservationsGUI.Rating")
	};
	private  JLabel errorlbl = new JLabel(/*ResourceBundle.getBundle("Etiquetas").getString("BidaiakErreserbatuGUI.lblNewLabel.text")*/); //$NON-NLS-1$ //$NON-NLS-2$


	public BidaiakErreserbatuGUI(Traveller t)
	{
		this.traveller=t;
		...
		jButtonClose.setBounds(new Rectangle(116, 419, 78, 16));
		...
		setTitle(ResourceBundle.getBundle("Etiquetas").getString("BidaiakErreserbatuGUI.MainTitle") + " - traveller :"+traveller.getName());
		...
		jCalendar1.addPropertyChangeListener(new PropertyChangeListener()
		{
			...
			{
				btnErreserbatu.setEnabled(false);
				seatslagun.removeAllElements();
				...
				else if (propertychangeevent.getPropertyName().equals("calendar"))
				{
					...
					try {
						...
						for (domain.Ride ride:rides){
							...
							row.add(ride.getDriver().getUser()+"/"+ride.getDriver().getName());
							row.add(ride.lortuEserlekuKopMin((String)jComboBoxOrigin.getSelectedItem(),(String)jComboBoxDestination.getSelectedItem()));
							row.add(ride.lortuBidaiarenPrezioa((String)jComboBoxOrigin.getSelectedItem(),(String)jComboBoxDestination.getSelectedItem()));
							...
							float media= blf.getBalorazioMedia(ride.getDriver());
							row.add(media);
		...
		tableRides.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				rowClicked=tableRides.getSelectedRow();
				btnErreserbatu.setEnabled(true);
				int seats=(int) tableModelRides.getValueAt(rowClicked, 1);
				kargatuLaguntzaileBox(seats);
				seatsBox.setVisible(true);
				
				seatKoplbl.setText(ResourceBundle.getBundle("Etiquetas").getString("BidaiakErreserbatuGUI.seatKop"));
			}
		});
		...
		paintDaysWithEvents(jCalendar1,datesWithRidesCurrentMonth,Color.CYAN);
		 ...
		 btnErreserbatu.addActionListener(new ActionListener() {
		 	public void actionPerformed(ActionEvent e) {
		 		Ride ride=(Ride) tableModelRides.getValueAt(rowClicked, 3);
		 		BLFacade facade = MainGUI.getBusinessLogic();
		 		int seats= (int) seatsBox.getSelectedItem();
		 		float dirua= facade.getMoney(traveller);
		 		String from=(String)jComboBoxOrigin.getSelectedItem();
		 		String to=(String)jComboBoxDestination.getSelectedItem();
		 		if(seats*ride.lortuBidaiarenPrezioa(from, to)>dirua){
		 			//IDATZI GUIAN GORRIZ.
		 			errorlbl.setText(ResourceBundle.getBundle("Etiquetas").getString("FindRidesGUI.errorNotEnoughMoney")+ dirua+"€");
		 			
		 			//System.out.println("Ez duzu diru nahikorik: "+dirua +"€ dituzu");
		 		}else {
		 			Date orain=  new Date();
			 		BLFacade blf= MainGUI.getBusinessLogic();
			 		...
			 		if(request==null) {
			 			...
			 			//System.out.println("Jadanik erreserbatuta bidaia hori");
		...
		btnErreserbatu.setBounds(298, 417, 102, 36);
		btnErreserbatu.setEnabled(false);
		...
		errorlbl.setBounds(22, 194, 252, 25);
		
		getContentPane().add(errorlbl);
		
		 //$NON-NLS-1$ //$NON-NLS-2$
		seatKoplbl.setBounds(52, 137, 102, 21);
		getContentPane().add(seatKoplbl);
		
		seatsBox.setModel(seatslagun);
		seatsBox.setVisible(false);
		seatsBox.setBounds(190, 137, 61, 21);
		getContentPane().add(seatsBox);
		alertaBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				JFrame a= new AlertaGUI(traveller);
				a.setVisible(true);
			}
		});
	 ...
	 //$NON-NLS-1$ //$NON-NLS-2$
		...
		alertaBtn.setBounds(35, 289, 106, 21);
		getContentPane().add(alertaBtn);
	...
	private void kargatuLaguntzaileBox(int kop) {
		seatslagun.removeAllElements();
		for(int i=1; i<=kop;i++) {
			seatslagun.addElement(i);
		}
	}
}
