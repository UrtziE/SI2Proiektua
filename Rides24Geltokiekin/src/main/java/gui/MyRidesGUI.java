...
public class MyRidesGUI extends JFrame {
	...
	public MyRidesGUI(Driver p) {
		...
		lblNewLabel_5.setBounds(303, 202, 79, 13);
		...
		seatlbl.setBounds(304, 236, 126, 13);
		contentPane.add(seatlbl);

		// $NON-NLS-1$ //$NON-NLS-2$
		seatKop.setBounds(446, 236, 45, 13);
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (ride != null) {
					...
					Gidarialbl.setText(driver.getFullName());
						...
						if( rdbtnEzEginak.isSelected()) {
							 kargatuTravellerakNotDone(ride);
						}
						if(  rdbtnEgindakoak .isSelected()) {
							 kargatuTravellerakDone(ride);
						}
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (r != null) {
					...
					boolean baloratu = true;
						...
						Traveller t = (Traveller) comboBoxTraveller.getSelectedItem();
						if (t != null) {
							//
							
							BLFacade blf = MainGUI.getBusinessLogic();
							request= blf.lortuLehenRequestBidaiakoa(r,t);
							//request = t.lortuLehenRequestBidaiakoa(r);
							
							nori=t;
							if (blf.isBaloratua(request, true)) {
								baloratu = false;
							}
						}
				...
				if (r != null) {
					...
					boolean erreklamatu = true;
						...
						if (t != null) {
							BLFacade blf = MainGUI.getBusinessLogic();
							request= blf.lortuLehenRequestBidaiakoa(r,t);
							if (blf.isErreklamatua(request, true)) {
								erreklamatu = false;
							}
						}
		...
		ibilbidelbl.setBounds(304, 271, 103, 13);
		contentPane.add(ibilbidelbl);

		// $NON-NLS-1$ //$NON-NLS-2$
		ibilbidea.setBounds(392, 271, 181, 13);
	...
	private void kargatuTravellerakDone(Ride r) {
		 comboBoxTraveller.setVisible(true);
		travellers.removeAllElements();

		BLFacade blf = MainGUI.getBusinessLogic();
		List<Traveller> tList = blf.getTravellersOfRideDone(r);
		travellers.addAll(tList);
		if(travellers.getSize()!=0) {
			comboBoxTraveller.setSelectedIndex(0);
		}
	}
	private void kargatuTravellerakNotDone(Ride r) {
		 comboBoxTraveller.setVisible(true);
		travellers.removeAllElements();

		BLFacade blf = MainGUI.getBusinessLogic();
		List<Traveller> tList = blf.getTravellersOfRideNotDone(r);
	...
	private void kargatuEzEginak() {
		...
		List<Ride> rides= new LinkedList<Ride>();
			for (RideContainer ride : blf.getAllRidesOfDriver((Driver) profile)) {
				Ride r= ride.getRide();
				if (r.containsRequestNotDone()&&!rides.contains(r)) {
					rides.add(r);
	...
	private void kargatuKantzelatuak() {
		...
		for (RideContainer ride : blf.getAllRidesOfDriver((Driver) profile)) {
			Ride r= ride.getRide();
				if (r.getEgoera().equals(EgoeraRide.KANTZELATUA)&&!rides.contains(r)) {
					rides.add(r);
				}
			}
		laguntzaile.addAll(rides);
...
//Aldatu
	private void kargatuEgindakoak() {
		...
		for (RideContainer ride : blf.getAllRidesOfDriver((Driver) profile)) {
			Ride r= ride.getRide();
				if (r.containsRequestDone()&&!rides.contains(r)) {
					rides.add(r);
				}
			}
		laguntzaile.addAll(rides);
	...
	private void kargatuTratatuGabeak() {
		...
		for (RideContainer ride : blf.getAllRidesOfDriver((Driver) profile)) {
			Ride r= ride.getRide();
				if (r.getEgoera().equals(EgoeraRide.PASATUA)&&!rides.contains(r)) {
					rides.add(r);
				}
			}
	...
	private void kargatuHurrengoak() {
		...
		for (RideContainer ride : blf.getAllRidesOfDriver((Driver) profile)) {
			Ride r= ride.getRide();
				if (r.getEgoera().equals(EgoeraRide.MARTXAN)&&!rides.contains(r)) {
					rides.add(r);
				}
			}
		laguntzaile.addAll(rides);
